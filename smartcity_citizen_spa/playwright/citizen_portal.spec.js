...
const SPA_URL = 'file:///C:/Users/Lenovo/ALDIII/lab15%20R/smartcity_citizen_spa/index.html';
...
// ---------------------------------------------------------------------------
const TEST_CITIZEN_USERNAME = 'rey';
const TEST_CITIZEN_PASSWORD = '@Aldo0306';
const TEST_ADMIN_USERNAME  = 'aldo';
const TEST_ADMIN_PASSWORD  = '@Aldo0306';
 ...
 *
 * Fungsi ini akan mencegat request ke domain apapun dan membelokkannya
 * secara paksa ke server Django lokal di http://localhost:8000/api.
 *
 * @param {import('@playwright/test').Page} page - Objek halaman Playwright
 */
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    test('UI-03: Pagination Feed Kota — maks 10 kartu, kontrol pagination muncul', async ({ page }) => {
        ...
        await page.waitForSelector('button:has-text("Laporan Baru")', { state: 'visible', timeout: 10000 });
        ...
        const listContainer = page.locator('#report-container');
    ...
    test('UI-04: Klik tombol Buat Laporan → modal #reportModal muncul', async ({ page }) => {
        ...
        const btnBukaModal = page.locator('button').filter({ hasText: 'Laporan Baru' });
        ...
        await expect(page.locator('#btn-submit-report')).toBeVisible();
    ...
    test('UI-05: Isi form dan simpan draft → modal tutup, notifikasi muncul', async ({ page }) => {
        ...
        const draftBadge = page.locator('#sidebar-draft-count');
